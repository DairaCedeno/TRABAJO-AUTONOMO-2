...
export const updateCliente = async (id: number, nombre?: string, apellido?: string, CI?: string, telefono?: string, direccion?: string, email?: string) => {
  const response = await axios.put(`${API_BASE_URL}/clientes/${id}`, { nombre, apellido, CI, telefono, direccion, email });
  return response.data;
};
